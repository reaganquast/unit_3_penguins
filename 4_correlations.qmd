...
So for bill length vs. bill depth, Pearson's r = 0.64. That's a pretty solid correlation coefficient, so those two variables definitely have a (positive) linear relationship. To conduct hypothesis testing with a correlation test, we set the null hypothesis as **r=0**, and the alternative hypothesis is that **r != 0**. The p-value is the likelihood of obtaining the sample data (our penguin bill measurements) if the null hypothesis is true. If the p-value is very small (typically set to <0.05), then the chance of the null hypothesis being true is very unlikely, so we accept the alternative hypothesis. From cor.test() we can see that the p-value is much less than 0.05, so the correlation is "significant". 
