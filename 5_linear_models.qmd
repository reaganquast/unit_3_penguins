...
-  Linear relationship 
-  Normality of the model residuals (i.e. the difference between the observed data and the model prediction)
-  No or little multicollinearity (although this depends on your goals)
...
-  Square root of Standardized residuals vs Fitted values, this is very similar to number 1, where the Y axis of residuals is in a different metric.
...
ggplot() +
     ...
     geom_line(aes(y = fit, x = bill_length_mm, color=species), linewidth = 1, data=newdata_predict_lm_3) +
     theme_bw()
```
...
ggplot() +
  ...
  geom_line(data=lm_3_predict, aes(y = .fitted, x = bill_length_mm, color=species), linewidth = 1)
```
...
We can use the AIC() function to compare the Akaike Information Criterion between the 2 models. AIC can be used to compare 2 different models (they do not have to be "nested" models) as long as the models were estimated using the same dataset. The AIC for `lm_4` is higher, meaning the fitness is poorer compared to `lm_3`. Another option for comparing a complex model with a simpler model is to use the `step()` function. You can input the most complicated model formulation that you are intersted in and this function will reduce the model's complexity one step at a time and check the fitness at each step. The output for each "step" of the `step()` function shows the change in AIC if each potential single term is dropped from the model, listed from the best model to the worst model. In this case, step 1 shows that dropping the interaction term produces a better model (a lower AIC value) than if nothing was dropped. In step 2, the best outcome is to drop nothing, so the model is step 2 is returned as the best model. Read the documentation on `step()` to learn more about how decisions are made.

```{r}
AIC(lm_3, lm_4) # note df is the number of parameters estimated in the model (including the error term)
...
ggplot(data=lm_4_predict) +
  ...
  geom_line(aes(y = .fitted, x = bill_length_mm, color=species), linewidth = 1) + 
  theme_bw()
```
...
Analysis of Variance (ANOVA) is just a special case of linear regression where the independent predictor variables are all categorical variables. Mathematically, running an ANOVA is the same as running a linear model. If you plan to include continuous (numerical) predictor variables, this is called ANCOVA (Analysis of Covariance). To run an ANOVA / ANCOVA, you can build your linear model using `lm()` as before, and then print the output of the model using the function `anova()`, which presents the statistical output in a classic ANOVA table. Another way to run an ANOVA is to use the `aov()` function to run an ANOVA  model directly. 
